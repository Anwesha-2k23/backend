...
class Login(APIView):
    def get(self, request):
        ...
        try:
            ...
            return JsonResponse(
                {"message": "Your token is expired please generate new one"}, status=409
            )
        ...
        try:
            ...
            if user.is_email_verified == True:
                ...
                return JsonResponse(
                    {
                        "message": "Your Email is not verified please verify email to continue further"
                    },
                    status=403,
                )
        except:
        ...
        if user:
            if this_user.is_email_verified == True:
                ...
                response.data = {
                    ...
                    + "/"
                    + AWS_PUBLIC_MEDIA_LOCATION2
                    + str(this_user.qr_code),
                }
                response.set_cookie(
                    key="jwt", value=token, httponly=False, samesite="None", secure=True
                )
                return response
            else:
                return JsonResponse(
                    {
                        "message": "Please verify your email through Slick app to continue"
                    },
                    status=403,
                )
    ...
    def post(self, request):
        ...
        try:
            ...
            if User.objects.filter(phone_number=phone_number).exists():
                return JsonResponse(
                    {
                        "message": "An User  With same Phone Number already exists",
                        "status": "409",
                    },
                    status=409,
                )
            ...
            return JsonResponse(
                {"message": "required form data not recived"}, status=401
            )
        itime = time.time()
...
class editProfile(APIView):
    ...
    def post(self, request, **kwargs):
        ...
        user.save()
        response = Response()
        response.data = {"message": "Profile successfully updated!"}
        return response
        ...
        try:
            ...
            if User.objects.filter(email_id=email).exists():
                try:
                    ...
                    token = jwt.encode(
                        payload=payload, key=COOKIE_ENCRYPTION_SECRET, algorithm="HS256"
                    )  # not working ??
                    return JsonResponse({"token": token}, status=201)
                except:
                    return JsonResponse(
                        {"message": "Token cannot be generated"}, status=409
                    )
            else:
...
class ForgetPassword(APIView):
    ...
    def post(self, request):  # posting the email address // mail the link
        ...
        try:
            ...
            ## send mail
            send_email_using_microservice(
                email_id=request.data["email"], subject="Change password", text=text
            )
...
class GetQRData(APIView):
    def get(self, request, **kwargs):
        ...
        return JsonResponse({"qr_data": user.signature}, safe=False)
